...
const EVENT_TYPES = {
  walk:     { icon: '🐾', label: 'Прогулка',      color: '#7ED321' },
  meetup:   { icon: '🐕', label: 'Встреча',        color: '#F5A623' },
  training: { icon: '🎓', label: 'Тренировка',     color: '#4A90D9' },
  promo:    { icon: '🎁', label: 'Акция',           color: '#9B59B6' },
  openday:  { icon: '🏥', label: 'Открытый день',  color: '#E74C3C' },
};
...
function renderHomeEvents() {
  ...
  row.innerHTML = upcoming.map(ev => {
    ...
    return `<div class="ec" onclick="openEventDetail('${ev.id}')">
      ...
      <div style="font-size:12px;color:var(--text-secondary);">📍 ${ev.address}</div>
      <div style="font-size:12px;color:var(--text-secondary);">🕐 ${formatEventDate(ev.event_date)}</div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:2px;">
        <span style="font-size:12px;color:var(--text-secondary);">👥 ${ev.participants_count || 0} идут</span>
  ...
  list.innerHTML = filtered.map(ev => {
    ...
    return `<div class="ev-full-card" onclick="openEventDetail('${ev.id}')">
      <div style="display:flex;align-items:flex-start;gap:12px;">
        <div style="width:48px;height:48px;border-radius:14px;background:${t.color}22;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;">${t.icon}</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding-top:10px;border-top:1px solid var(--border);">
        <div style="display:flex;align-items:center;gap:8px;">
          <span style="font-size:13px;font-weight:700;">👥 ${ev.participants_count || 0}${ev.max_participants ? '/' + ev.max_participants : ''}</span>
